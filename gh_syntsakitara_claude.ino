...
const char * SET_NAMES[3] = { "Pop", "Rock", "Balladi" };
const char * OCT_NAMES[3] = { "Basso", "Normaali", "Melodia" };
...
bool fretHeld[5] = { false };
bool strumDownLast = true; // Alustus TRUE, koska PULLUP-tilassa lepää HIGH-arvossa
bool strumUpLast = true;
...
void setup() {
   ...
   // Asetettu turvalliseen PULLUP-tilaan (oletetaan maadoituskytkentä)
   for (int i = 0; i < 5; i++)
...
void updateControl() {
   ...
   for (int i = 0; i < 5; i++) {
      bool pressed = digitalRead(FRET_PINS[i]) == LOW; // PULLUP: Painallus on LOW
      if (!pressed && fretHeld[i])
         chordMode ? releaseChord(i) : releaseNote(i);
      fretHeld[i] = pressed;
   }

   bool sd = digitalRead(PIN_STRUM_D) == LOW; 
   bool su = digitalRead(PIN_STRUM_U) == LOW; 
   ...
   bool sta = digitalRead(PIN_START) == LOW; 
   if (sta && !startLast) {
      if (chordMode) {
   ...
   bool dp = digitalRead(PIN_DPAD) == LOW; 
   if (dp && !dpadLast) {
      vibratoOn = !vibratoOn;
   ...
   // Whammy: Lepää nollassa, kampea painamalla arvo nousee max 4095
   int whammy = mozziAnalogRead(PIN_WHAMMY);
   float bend = (float)whammy / 4095.0f * 0.05f; // Korjattu syntaksi
   
   float vibrato = 0;
   if (vibratoOn) {
      vibratoPhase = (vibratoPhase + 1) % CONTROL_RATE;
      vibrato = 0.015f * sin(2.0f * 3.14159f * vibratoPhase /
         (float)CONTROL_RATE * 5.0f); // Korjattu syntaksi
   }
   ...
   for (int i = 0; i < 5; i++) {
      for (int n = 0; n < 3; n++) {
         ...
         if (base > 0)
            osc[idx].setFreq(base * (1.0f + bend + vibrato)); // Korjattu syntaksi
         env[idx].update();
      }
   }
}
...
AudioOutput_t updateAudio() {
   ...
   // Jaetaan 15:llä, jotta kaikkien oskillaattorien yhtäaikainen soiminen ei säröydy
   return MonoOutput::fromAlmostNBit(11, out / 15);
}
